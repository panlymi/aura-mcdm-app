...
## Complete formula-driven Excel exports

After an AURA, SYAI, ARIE, WASPAS, MOORA, SAW, TOPSIS, or VIKOR calculation,
the Results tab provides a complete `.xlsx` workbook rather than a flat ranking table.
...
- a **MOORA** sheet with live vector normalization formulas, weighted normalized
  components, benefit and cost sums, normalized assessment values ($y_i$),
  competition ranks, and a tie-safe score-sorted final table;
...
The SAW workbook contains live benefit/cost ratio normalization, automatically
normalized weights, weighted contributions, additive scores, competition ranks,
a tie-safe sorted table, a summary chart, verified values, and a formula guide.

The TOPSIS workbook contains live vector normalization, weighted positive and
negative ideals, Euclidean separation measures, relative closeness, competition
ranks, a tie-safe sorted table, a summary chart, verified values, and a formula guide.

The VIKOR workbook contains an editable `v` control, live best/worst references,
normalized and weighted loss matrices, group utility (`S_i`), individual regret
(`R_i`), compromise index (`Q_i`), a tie-safe sorted table, a summary chart,
verified values, and a formula guide. It mirrors the app's `Q_i` ranking and does
not infer an additional compromise set from acceptance conditions.

All eight workbooks follow the research-sheet visual convention: orange weight rows,
...
kappa cells, WASPAS's highlighted lambda cell, VIKOR's highlighted `v` cell, or
criterion weights refreshes the live scores, ranks, summary KPIs, and chart.
